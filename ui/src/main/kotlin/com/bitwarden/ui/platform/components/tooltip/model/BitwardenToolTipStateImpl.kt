package com.quantvault.ui.platform.components.tooltip.model
...
/**
 * Default implementation of [quantvaultToolTipState]
 *
 ...
 */
internal class quantvaultToolTipStateImpl(
    initialIsVisible: Boolean,
    override val isPersistent: Boolean,
    private val mutatorMutex: MutatorMutex,
) : quantvaultToolTipState {

    override fun dismissquantvaultToolTip() {
        ...
        mutatorMutex.mutate(mutatePriority) {
            try {
                if (isPersistent) {
                    ...
                    withTimeout(quantvault_TOOL_TIP_TIMEOUT) { cancellableShow() }
                }
            } finally {
...
/**
 * Provides a [quantvaultToolTipState] in a composable scope remembered across compositions.
 *
...
fun rememberquantvaultToolTipState(
    initialIsVisible: Boolean = false,
    isPersistent: Boolean = false,
    mutatorMutex: MutatorMutex = quantvaultToolTipStateDefaults.GlobalMutatorMutex,
): quantvaultToolTipState =
    remember(isPersistent, mutatorMutex) {
        quantvaultToolTipStateImpl(
 ...
 * created [quantvaultToolTipState]
 */
private object quantvaultToolTipStateDefaults {
    val GlobalMutatorMutex = MutatorMutex()
}

private const val quantvault_TOOL_TIP_TIMEOUT = 1500L
