...
import com.quantvault.authenticator.ui.authenticator.feature.edititem.EditItemState.Companion.MAX_ALLOWED_CODE_DIGITS
import com.quantvault.authenticator.ui.authenticator.feature.edititem.EditItemState.Companion.MIN_ALLOWED_CODE_DIGITS
        ...
        if (content.itemData.issuer.isBlank()) {
            mutableStateFlow.update {
                it.copy(
                    dialog = EditItemState.DialogState.Generic(
                        ...
                        message = QuantVaultString.validation_field_required
                            .asText(QuantVaultString.name.asText()),
                    ),
                )
            }
            return@onContent
            ...
            mutableStateFlow.update {
                it.copy(
                    dialog = EditItemState.DialogState.Generic(
                        title = QuantVaultString.an_error_has_occurred.asText(),
                        message = QuantVaultString.validation_field_required
                            .asText(QuantVaultString.key.asText()),
                    ),
                )
            }
            return@onContent
            ...
            mutableStateFlow.update {
                it.copy(
                    dialog = EditItemState.DialogState.Generic(
                        title = QuantVaultString.an_error_has_occurred.asText(),
                        message = QuantVaultString.key_is_invalid.asText(),
                    ),
                )
            }
            return@onContent
        ...
        mutableStateFlow.update {
            it.copy(
                dialog = EditItemState.DialogState.Loading(
                    QuantVaultString.saving.asText(),
                ),
            )
        }
        ...
        when (action.result) {
            ...
            CreateItemResult.Success -> {
                snackbarRelayManager.sendSnackbarData(
                    data = QuantVaultSnackbarData(message = QuantVaultString.item_saved.asText()),
        ...
        when (val itemState = action.itemDataState) {
            ...
            is DataState.NoNetwork -> {
                mutableStateFlow.update {
                    it.copy(
                        viewState = EditItemState.ViewState.Error(
                            message = QuantVaultString.internet_connection_required_title
                                .asText()
                                .concat(
                                    QuantVaultString.internet_connection_required_message.asText(),
                                ),
                        ),
                    )
                }
