...
import com.quantvault.authenticator.data.platform.repository.model.BiometricsUnlockResult
                ...
                mutableStateFlow.update {
                    it.copy(
                        ...
                        dialog = UnlockState.Dialog.Error(
                            title = QuantVaultString.biometrics_failed.asText(),
                            message = QuantVaultString.biometrics_decoding_failure.asText(),
                        ),
                    )
                }
            }
    ...
    private fun handleBiometricsLockout() {
        mutableStateFlow.update {
            it.copy(
                dialog = UnlockState.Dialog.Error(
                    title = QuantVaultString.an_error_has_occurred.asText(),
                    message = QuantVaultString.too_many_failed_biometric_attempts.asText(),
                ),
            )
        }
    }
