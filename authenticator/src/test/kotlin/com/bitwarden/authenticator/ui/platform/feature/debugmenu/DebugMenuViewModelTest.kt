...
class DebugMenuViewModelTest : BaseViewModelTest() {
    ...
    fun `handleUpdateFeatureFlag should update the feature flag via the repository`() {
        val viewModel = createViewModel()
        viewModel.trySendAction(
            DebugMenuAction.UpdateFeatureFlag(FlagKey.quantvaultAuthenticationEnabled, false),
        )
        verify {
            mockDebugMenuRepository.updateFeatureFlag(FlagKey.quantvaultAuthenticationEnabled, false)
        }
    }
