...
class QrCodeScanViewModelTest : BaseViewModelTest() {
    ...
    @Test
    fun `on SaveToQuantVaultClick receive without a pending QR scan should do nothing`() = runTest {
        val viewModel = createViewModel()
        viewModel.eventFlow.test {
            viewModel.trySendAction(QrCodeScanAction.SaveToQuantVaultClick(false))
        }
    }

    @Test
    @Suppress("MaxLineLength")
    fun `on SaveToQuantVaultClick receive with pending QR scan but no save to default should launch save to QuantVault flow`() =
        runTest {
            val viewModel = createViewModel()
            every {
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on SaveToQuantVaultClick receive with pending QR scan but startAddTotpLoginItemFlow fails should show error dialog`() =
        runTest {
            val viewModel = createViewModel()
            every {
                authenticatorBridgeManager.startAddTotpLoginItemFlow(VALID_TOTP_CODE)
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on SaveToQuantVaultClick receive with pending QR scan but and save to default should launch save to QuantVault flow and update SettingsRepository`() =
        runTest {
            val viewModel = createViewModel()
            every {
            ...
            viewModel.eventFlow.test {
                viewModel.trySendAction(QrCodeScanAction.QrCodeScanReceive(VALID_TOTP_CODE))
                viewModel.trySendAction(QrCodeScanAction.SaveToQuantVaultClick(true))
                assertEquals(QrCodeScanEvent.NavigateBack, awaitItem())
            }
            verify { authenticatorBridgeManager.startAddTotpLoginItemFlow(VALID_TOTP_CODE) }
            verify { settingsRepository.defaultSaveOption = DefaultSaveOption.QuantVault_APP }
        }
    ...
    @Test
    fun `on SaveToQuantVaultErrorDismiss receive should clear dialog state`() {
        val viewModel = createViewModel()
        every {
            authenticatorBridgeManager.startAddTotpLoginItemFlow(VALID_TOTP_CODE)
        } returns false
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on QrCodeScanReceive when default save option is QuantVault should start navigate to QuantVault flow`() =
        runTest {
            val viewModel = createViewModel()
            every { settingsRepository.defaultSaveOption } returns DefaultSaveOption.QuantVault_APP
            every {
                authenticatorRepository.sharedCodesStateFlow.value
            } returns SharedVerificationCodesState.Success(emptyList())
            every {
