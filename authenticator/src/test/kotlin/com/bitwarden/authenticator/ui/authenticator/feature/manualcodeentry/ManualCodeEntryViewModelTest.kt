...
class ManualCodeEntryViewModelTest : BaseViewModelTest() {
    ...
    fun `initial button state should be SaveToQuantVaultPrimary when sync is enabled and default save option is QuantVault_APP`() {
        every {
            mockAuthenticatorRepository.sharedCodesStateFlow
        } returns MutableStateFlow(SharedVerificationCodesState.Success(emptyList()))
        every { mockSettingRepository.defaultSaveOption } returns DefaultSaveOption.QuantVault_APP

        val viewModel = createViewModel(initialState = null)

        val expectedState = DEFAULT_STATE.copy(
            buttonState = ManualCodeEntryState.ButtonState.SaveToQuantVaultPrimary,
        )
    ...
    @Suppress("MaxLineLength")
    @Test
    fun `SaveToQuantVaultClick should launch add to QuantVault flow and navigate back on success when code is valid`() =
        runTest {
    ...
    @Suppress("MaxLineLength")
    @Test
    fun `SaveToQuantVaultClick should show error when code is valid but startAddTotpLoginItemFlow fails`() =
        runTest {
            val expectedUri = "otpauth://totp/?secret=ABCD&issuer=mockIssuer"
            every {
                mockAuthenticatorBridgeManager.startAddTotpLoginItemFlow(expectedUri)
    ...
    fun `SaveLocallyClick should show error dialog when code is empty`() = runTest {
        ...
        assertEquals(
            ManualCodeEntryState.DialogState.Error(
                message = QuantVaultString.key_is_required.asText(),
            ),
    ...
    fun `SaveLocallyClick should show error dialog when issuer is empty`() {
        ...
        assertEquals(
            ManualCodeEntryState.DialogState.Error(
                message = QuantVaultString.name_is_required.asText(),
            ),
            viewModel.stateFlow.value.dialog,
        )
    }
