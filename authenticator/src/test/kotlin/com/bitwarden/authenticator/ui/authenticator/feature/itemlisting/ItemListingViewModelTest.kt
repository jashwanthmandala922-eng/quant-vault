...
import com.quantvault.authenticator.ui.authenticator.feature.util.toDisplayItem
import com.quantvault.authenticator.ui.authenticator.feature.util.toSharedCodesDisplayState
...
class ItemListingViewModelTest : BaseViewModelTest() {
    ...
    private val encodingManager: QuantVaultEncodingManager = mockk()
    private val settingsRepository: SettingsRepository = mockk {
        ...
        every { hasUserDismissedDownloadQuantVaultCard } returns false
    }
    ...
    fun `stateFlow value should show download QuantVault action card when local items are empty and shared state is AppNotInstalled`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = ItemListingState.ActionCardState.DownloadQuantVaultApp,
            ),
        )
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should not show download QuantVault card when local items are empty and shared state is AppNotInstalled but user has dismissed card`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = null,
            ),
        )
        every { settingsRepository.hasUserDismissedDownloadQuantVaultCard } returns true
        mutableSharedCodesFlow.value = SharedVerificationCodesState.AppNotInstalled
        mutableVerificationCodesFlow.value = DataState.Loaded(emptyList())
        val viewModel = createViewModel()
        assertEquals(expectedState, viewModel.stateFlow.value)
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should show download QuantVault card when there are local items and shared state is AppNotInstalled`() {
        val expectedState = DEFAULT_STATE.copy(
        ...
        every { settingsRepository.hasUserDismissedDownloadQuantVaultCard } returns false
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should not show download QuantVault card when there are local items and shared state is AppNotInstalled but user has dismissed card`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = null,
                favoriteItems = LOCAL_FAVORITE_ITEMS,
    ...
    fun `stateFlow sharedItems value should be Codes with empty list when shared state is Success `() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = null,
                favoriteItems = LOCAL_FAVORITE_ITEMS
                    .map { it.copy(showMoveToQuantVault = true) }
                    .toImmutableList(),
                itemList = LOCAL_NON_FAVORITE_ITEMS
                    .map { it.copy(showMoveToQuantVault = true) }
                    .toImmutableList(),
    ...
    @Test
    fun `on DownloadQuantVaultClick receive should emit NavigateToQuantVaultListing`() = runTest {
        val viewModel = createViewModel()
        viewModel.eventFlow.test {
            viewModel.trySendAction(ItemListingAction.DownloadQuantVaultClick)
            assertEquals(ItemListingEvent.NavigateToQuantVaultListing, awaitItem())
        }
    }

    @Test
    @Suppress("MaxLineLength")
    fun `on DownloadQuantVaultDismiss receive should dismiss action card and store dismissal in settings`() =
        runTest {
            val expectedState = DEFAULT_STATE.copy(
                viewState = ItemListingState.ViewState.Content(
                    actionCard = null,
            ...
            every { settingsRepository.hasUserDismissedDownloadQuantVaultCard = true } just runs
            every { settingsRepository.hasUserDismissedDownloadQuantVaultCard } returns false
            mutableVerificationCodesFlow.value = DataState.Loaded(LOCAL_VERIFICATION_ITEMS)
            val viewModel = createViewModel()
            viewModel.trySendAction(ItemListingAction.DownloadQuantVaultDismiss)
            verify { settingsRepository.hasUserDismissedDownloadQuantVaultCard = true }
            assertEquals(expectedState, viewModel.stateFlow.value)
        }

    @Test
    @Suppress("MaxLineLength")
    fun `on DownloadQuantVaultDismiss receive in empty state should dismiss action card and store dismissal in settings`() =
        runTest {
            val expectedState = DEFAULT_STATE.copy(
                viewState = ItemListingState.ViewState.NoItems(
                    actionCard = null,
    ...
    @Test
    fun `on SyncWithQuantVaultClick receive should emit NavigateToQuantVaultSettings`() = runTest {
        val viewModel = createViewModel()
        viewModel.eventFlow.test {
            viewModel.trySendAction(ItemListingAction.SyncWithQuantVaultClick)
            assertEquals(ItemListingEvent.NavigateToQuantVaultSettings, awaitItem())
        }
    }

    @Test
    @Suppress("MaxLineLength")
    fun `on SyncWithQuantVaultDismiss receive should dismiss action card and store dismissal in settings`() =
        runTest {
            val expectedState = DEFAULT_STATE.copy(
                viewState = ItemListingState.ViewState.Content(
                    actionCard = null,
            ...
            every { settingsRepository.hasUserDismissedSyncWithQuantVaultCard = true } just runs
            every { settingsRepository.hasUserDismissedSyncWithQuantVaultCard } returns false
            mutableVerificationCodesFlow.value = DataState.Loaded(LOCAL_VERIFICATION_ITEMS)
            val viewModel = createViewModel()
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on SyncWithQuantVaultDismiss receive in empty state should dismiss action card and store dismissal in settings`() =
        runTest {
            val expectedState = DEFAULT_STATE.copy(
                viewState = ItemListingState.ViewState.NoItems(
                    actionCard = null,
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should show sync with QuantVault action card when local items are empty and shared state is SyncNotEnabled`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = ItemListingState.ActionCardState.SyncWithQuantVault,
            ),
        )
        every { settingsRepository.hasUserDismissedSyncWithQuantVaultCard } returns false
        mutableSharedCodesFlow.value = SharedVerificationCodesState.SyncNotEnabled
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should not show download QuantVault card when local items are empty and shared state is SyncNotEnabled but user has dismissed card`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = null,
            ),
        )
        every { settingsRepository.hasUserDismissedSyncWithQuantVaultCard } returns true
        mutableSharedCodesFlow.value = SharedVerificationCodesState.SyncNotEnabled
        mutableVerificationCodesFlow.value = DataState.Loaded(emptyList())
        val viewModel = createViewModel()
        assertEquals(expectedState, viewModel.stateFlow.value)
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should show sync with QuantVault card when there are local items and shared state is SyncNotEnabled`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = ItemListingState.ActionCardState.SyncWithQuantVault,
                favoriteItems = LOCAL_FAVORITE_ITEMS,
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `stateFlow value should not show sync with QuantVault card when there are local items and shared state is AppNotInstalled but user has dismissed card`() {
        val expectedState = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = null,
                favoriteItems = LOCAL_FAVORITE_ITEMS,
    ...
    @Test
    fun `on CopyToQuantVaultClick receive should call startAddTotpLoginItemFlow`() {
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on CopyToQuantVaultClick should show error dialog when startAddTotpLoginItemFlow returns false`() {
        val expectedState = DEFAULT_STATE.copy(
        ...
        viewModel.eventFlow.test {
            ...
            assertEquals(
                ItemListingEvent.ShowSnackbar(
                    message = QuantVaultString.account_synced_from_QuantVault_app.asText(),
