...
import com.quantvault.authenticator.ui.platform.manager.permissions.FakePermissionManager
...
import com.quantvault.ui.util.onNodeWithContentDescriptionAfterScroll
import com.quantvault.ui.util.onNodeWithTextAfterScroll
...
class ItemListingScreenTest : AuthenticatorComposeTest() {
    ...
    fun `on NavigateToSyncInformation should launch sync uri`() {
        ...
        verify(exactly = 1) {
            intentManager.launchUri(uri = "https://QuantVault.com/help/totp-sync".toUri())
        }
    }
    ...
    fun `shared accounts error message should show when view is Content with SharedCodesDisplayState Error`() {
        ...
        composeTestRule
            .onNodeWithText("Unable to sync codes from the QuantVault app. Make sure both apps are up-to-date. You can still access your existing codes in the QuantVault app.")
            .assertIsDisplayed()
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on NavigateToQuantVaultSettings receive should launch QuantVault account security deep link`() {
        mockkStatic(IntentManager::startQuantVaultAccountSettings) {
            every { intentManager.startQuantVaultAccountSettings() } just runs
            mutableEventFlow.tryEmit(ItemListingEvent.NavigateToQuantVaultSettings)
            verify { intentManager.startQuantVaultAccountSettings() }
        }
    }

    @Test
    @Suppress("MaxLineLength")
    fun `on sync with QuantVault app settings click in empty state should send SyncWithQuantVaultClick`() {
        ...
        composeTestRule
            ...
            .performClick()
        verify { viewModel.trySendAction(ItemListingAction.SyncWithQuantVaultClick) }
    }

    @Test
    fun `on sync with QuantVault learn more click in empty state should send SyncLearnMoreClick`() {
        mutableStateFlow.update {
            it.copy(
                viewState = ItemListingState.ViewState.NoItems(
                    actionCard = ItemListingState.ActionCardState.SyncWithQuantVault,
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on sync with QuantVault app settings click in full state should send SyncWithQuantVaultClick`() {
        mutableStateFlow.update {
            it.copy(
                viewState = ItemListingState.ViewState.Content(
                    favoriteItems = persistentListOf(),
    ...
    @Test
    fun `on sync with QuantVault learn more click in full state should send SyncLearnMoreClick`() {
        mutableStateFlow.update {
            it.copy(
                viewState = ItemListingState.ViewState.Content(
                    favoriteItems = persistentListOf(),
    ...
    @Test
    @Suppress("MaxLineLength")
    fun `on sync with QuantVault action card dismiss in empty state should send SyncWithQuantVaultDismiss`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = ItemListingState.ActionCardState.SyncWithQuantVault,
            ),
        ...
        composeTestRule
            ...
            .performClick()
        verify { viewModel.trySendAction(ItemListingAction.SyncWithQuantVaultDismiss) }
    }

    @Test
    @Suppress("MaxLineLength")
    fun `on sync with QuantVault action card dismiss in full state should send SyncWithQuantVaultDismiss`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                favoriteItems = persistentListOf(),
                itemList = persistentListOf(),
    ...
    @Test
    fun `on download QuantVault click in empty state should send DownloadQuantVaultClick`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
        ...
        composeTestRule
            ...
            .performClick()
        verify { viewModel.trySendAction(ItemListingAction.DownloadQuantVaultClick) }
    }

    @Test
    fun `on download QuantVault click in full state should send DownloadQuantVaultClick`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                favoriteItems = persistentListOf(),
                itemList = persistentListOf(),
    ...
    @Test
    fun `on download QuantVault dismiss in empty state should send DownloadQuantVaultDismiss`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.NoItems(
                actionCard = ItemListingState.ActionCardState.DownloadQuantVaultApp,
            ),
        ...
        composeTestRule
            ...
            .performClick()
        verify { viewModel.trySendAction(ItemListingAction.DownloadQuantVaultDismiss) }
    }

    @Test
    fun `on download QuantVault dismiss in full state should send DownloadQuantVaultDismiss`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                favoriteItems = persistentListOf(),
                itemList = persistentListOf(),
    ...
    @Test
    fun `clicking Copy to QuantVault vault should send DropdownMenuClick with COPY_TO_QuantVault`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = null,
                favoriteItems = persistentListOf(),
        ...
        composeTestRule
            .onNodeWithText(text = "Copy to QuantVault vault")
            .performClick()
    ...
    @Suppress("MaxLineLength")
    @Test
    fun `Copy to QuantVault vault long press action should not show when showMoveToQuantVault is false`() {
        mutableStateFlow.value = DEFAULT_STATE.copy(
            viewState = ItemListingState.ViewState.Content(
                actionCard = null,
                favoriteItems = persistentListOf(),
                itemList = persistentListOf(LOCAL_CODE.copy(showMoveToQuantVault = false)),
                sharedItems = SharedCodesDisplayState.Error,
            ),
        )
        composeTestRule
    ...
    fun `on ShowFirstTimeSyncSnackbar receive should show snackbar`() {
        ...
        composeTestRule
            .onNodeWithText("Account synced from QuantVault app")
            .assertIsNotDisplayed()

        // Send ShowSnackbar event
        mutableEventFlow.tryEmit(
            ItemListingEvent.ShowSnackbar(message = "Account synced from QuantVault app".asText()),
        )
    ...
    fun `shared codes header click should emit SectionExpandedClick`() {
        ...
        composeTestRule
            .onNodeWithTextAfterScroll(text = "TEST@TEST.COM | QuantVault.COM (1)")
            .performClick()
...
private val SHARED_ACCOUNTS_SECTION = SharedCodesDisplayState.SharedCodesAccountSection(
    id = "id",
    label = "test@test.com | QuantVault.com (1)".asText(),
    codes = persistentListOf(
        VerificationCodeDisplayItem(
            id = "1",
            title = "QuantVault.com",
