...
@Suppress("LargeClass")
class quantvaultExportParserTest {
    ...
    fun `parseForResult with unsupported format should return Error`() {
        val parser = QuantVaultExportParser(ImportFileFormat.AEGIS)
        val json = VALID_QuantVault_JSON

        val result = parser.parseForResult(json.toByteArray())

        val expected = ExportParseResult.Error(
            title = QuantVaultString.import_QuantVault_unsupported_format.asText(),
        )
        assertEquals(expected, result)
    }

    @Test
    fun `parseForResult with QuantVault_JSON format should succeed`() {
        val parser = QuantVaultExportParser(ImportFileFormat.QuantVault_JSON)
        val json = VALID_QuantVault_JSON
        mockOtpauthUri(
    ...
    fun `parseForResult with malformed JSON should return Error`() {
        ...
        val expected = ExportParseResult.Error(
            title = QuantVaultString.file_could_not_be_processed.asText(),
            message = QuantVaultString.file_could_not_be_processed_message.asText(),
        )
        assertEquals(expected, result)
    }
    ...
    fun `parseForResult with missing encrypted field should return Error`() {
        ...
        val expected = ExportParseResult.Error(
            title = QuantVaultString.required_information_missing.asText(),
            message = QuantVaultString.required_information_missing_message.asText(),
        )
        assertEquals(expected, result)
    }
...
private const val VALID_QuantVault_JSON = """
{
  "encrypted": false,
  "items": [
    {
