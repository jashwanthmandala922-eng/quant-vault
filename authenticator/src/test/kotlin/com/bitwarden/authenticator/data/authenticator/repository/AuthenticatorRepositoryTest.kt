package com.quantvault.authenticator.data.authenticator.repository

import android.net.Uri
import app.cash.turbine.test
import com.quantvault.authenticator.data.authenticator.datasource.disk.util.FakeAuthenticatorDiskSource
import com.quantvault.authenticator.data.authenticator.datasource.entity.createMockAuthenticatorItemEntity
...
import com.quantvault.authenticator.data.authenticator.repository.model.ExportDataResult
...
import com.quantvault.authenticator.data.authenticator.repository.util.toAuthenticatorItems
...
import com.quantvault.core.data.util.mockBuilder
...
class AuthenticatorRepositoryTest {
    ...
    private val settingsRepository: SettingsRepository = mockk {
        every { previouslySyncedQuantVaultAccountIds } returns emptySet()
    }
    ...
    fun `firstTimeAccountSyncFlow should emit the first time an account syncs and update SettingsRepository`() =
        runTest {
            every { settingsRepository.previouslySyncedQuantVaultAccountIds = setOf("1") } just runs
            ...
            verify { settingsRepository.previouslySyncedQuantVaultAccountIds = setOf("1") }
        }

    @Test
    @Suppress("MaxLineLength")
    fun `firstTimeAccountSyncFlow should not emit if a synced account is already in previouslySyncedQuantVaultAccountIds`() =
        runTest {
            every { settingsRepository.previouslySyncedQuantVaultAccountIds } returns setOf("1")
            val sharedAccounts = listOf(
                SharedAccountData.Account(
                    userId = "1",
                    name = null,
