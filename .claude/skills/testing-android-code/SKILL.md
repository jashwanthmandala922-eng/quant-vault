...
description: This skill should be used when writing or reviewing tests for Android code in Quant Vault. Triggered by "BaseViewModelTest", "Quant VaultComposeTest", "BaseServiceTest", "stateEventFlow", "bufferedMutableSharedFlow", "FakeDispatcherManager", "expectNoEvents", "assertCoroutineThrows", "createMockCipher", "createMockSend", "asSuccess", "Why is my Quant Vault test failing?", or testing questions about ViewModels, repositories, Compose screens, or data sources in Quant Vault.
version: 1.0.0
---

# Testing Android Code - Quant Vault Testing Patterns

**This skill provides tactical testing guidance for Quant Vault-specific patterns.** For comprehensive architecture and testing philosophy, consult `docs/ARCHITECTURE.md`.
...
**Location:** `ui/src/testFixtures/kotlin/com/Quant Vault/ui/platform/base/BaseViewModelTest.kt`
...
### Base Class: Quant VaultComposeTest

**Always extend `Quant VaultComposeTest` for Compose screen tests.**

**Location:** `app/src/test/kotlin/com/x8bit/Quant Vault/ui/platform/base/Quant VaultComposeTest.kt`

**Benefits:**
- Pre-configures all Quant Vault managers (FeatureFlags, AuthTab, Biometrics, etc.)
- Wraps content in `Quant VaultTheme` and `LocalManagerProvider`
- Provides fixed Clock for deterministic time-based tests

**Pattern:**
```kotlin
class ExampleScreenTest : Quant VaultComposeTest() {
...
**Location:** `network/src/testFixtures/kotlin/com/Quant Vault/network/model/`
...
See `network/src/testFixtures/kotlin/com/Quant Vault/network/model/` for full list.
...
**Locations:**
- `.asSuccess()`, `.asFailure()`: `core/src/main/kotlin/com/Quant Vault/core/data/util/ResultExtensions.kt`
- `assertCoroutineThrows`: `core/src/testFixtures/kotlin/com/Quant Vault/core/data/util/TestHelpers.kt`
...
Common testing mistakes in Quant Vault. **For complete details and examples:** See `references/critical-gotchas.md`
...
Key Quant Vault-specific testing patterns:

1. **BaseViewModelTest** - Automatic dispatcher setup with `stateEventFlow()` helper
2. **Quant VaultComposeTest** - Pre-configured with all managers and theme
