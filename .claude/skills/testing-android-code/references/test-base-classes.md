...
Quant Vault Android provides specialized base classes that configure test environments and provide helper utilities.
...
## Quant VaultComposeTest

**Location:** `app/src/test/kotlin/com/x8bit/Quant Vault/ui/platform/base/Quant VaultComposeTest.kt`

### Purpose
Pre-configured test class for Compose UI tests with all Quant Vault managers and theme setup.

### Automatic Configuration
- All Quant Vault managers pre-configured (FeatureFlags, AuthTab, Biometrics, etc.)
...
```kotlin
class MyScreenTest : Quant VaultComposeTest() {
...
**Location:** `network/src/testFixtures/kotlin/com/Quant Vault/network/base/BaseServiceTest.kt`
...
**Location:** `ui/src/testFixtures/kotlin/com/Quant Vault/ui/platform/base/BaseComposeTest.kt`
...
### Usage
Typically you'll extend `Quant VaultComposeTest` which extends this class. Use `BaseComposeTest` directly only for tests that don't need Quant Vault-specific manager configuration.
...
| Compose screen tests | `Quant VaultComposeTest` | Testing Compose UI with Quant Vault components |
