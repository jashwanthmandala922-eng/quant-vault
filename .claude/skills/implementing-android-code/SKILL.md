...
version: 0.1.3
description: This skill should be used when implementing Android code in Quant Vault. Covers critical patterns, gotchas, and anti-patterns unique to this codebase. Triggered by "How do I implement a ViewModel?", "Create a new screen", "Add navigation", "Write a repository", "BaseViewModel pattern", "State-Action-Event", "type-safe navigation", "@Serializable route", "SavedStateHandle persistence", "process death recovery", "handleAction", "sendAction", "Hilt module", "Repository pattern", "implementing a screen", "adding a data source", "handling navigation", "encrypted storage", "security patterns", "Clock injection", "DataState", or any questions about implementing features, screens, ViewModels, data sources, or navigation in the Quant Vault Android app.
---

# Implementing Android Code - Quant Vault Quick Reference

**This skill provides tactical guidance for Quant Vault-specific patterns.** For comprehensive architecture decisions and complete code style rules, consult `docs/ARCHITECTURE.md` and `docs/STYLE_AND_BEST_PRACTICES.md`.
...
**Reference:**
- `ui/src/main/kotlin/com/Quant Vault/ui/platform/base/BaseViewModel.kt` (see `handleAction` method)
- `app/src/main/kotlin/com/x8bit/Quant Vault/ui/auth/feature/login/LoginViewModel.kt` (see class declaration)
...
**Reference:** `app/src/main/kotlin/com/x8bit/Quant Vault/ui/auth/feature/login/LoginNavigation.kt` (see `LoginRoute` and extensions)
...
fun ExampleScreen(
    ...
    Quant VaultScaffold(
        topBar = {
            Quant VaultTopAppBar(
                title = stringResource(R.string.title),
                navigationIcon = rememberVectorPainter(Quant VaultDrawable.ic_back),
...
**Reference:** `app/src/main/kotlin/com/x8bit/Quant Vault/ui/auth/feature/login/LoginScreen.kt` (see `LoginScreen` composable)
...
- ✅ Use `Quant Vault*` prefixed components from `:ui` module
...
**Reference:**
- `app/src/main/kotlin/com/x8bit/Quant Vault/data/auth/repository/AuthRepository.kt`
- `app/src/main/kotlin/com/x8bit/Quant Vault/data/tools/generator/repository/di/GeneratorRepositoryModule.kt`
...
The `:ui` module provides reusable `Quant Vault*` prefixed components. Search before creating new ones.

**Common Components:**
- `Quant VaultFilledButton` - Primary action buttons
- `Quant VaultOutlinedButton` - Secondary action buttons
- `Quant VaultTextField` - Text input fields
- `Quant VaultPasswordField` - Password input with show/hide
- `Quant VaultSwitch` - Toggle switches
- `Quant VaultTopAppBar` - Toolbar/app bar
- `Quant VaultScaffold` - Screen container with scaffold
- `Quant VaultBasicDialog` - Simple dialogs
- `Quant VaultLoadingDialog` - Loading indicators

**Component Discovery:**
Search `ui/src/main/kotlin/com/Quant Vault/ui/platform/components/` for existing `Quant Vault*` components. For build, test, and codebase discovery commands, use the **`build-test-verify`** skill.
...
- Prefix with `Quant Vault`
- Accept themed colors/styles from `Quant VaultTheme`
...
- Reference strings via generated `Quant VaultString` resource IDs
...
**Reference:** `app/src/test/kotlin/com/x8bit/Quant Vault/ui/tools/feature/generator/GeneratorViewModelTest.kt`
...
- `core/src/main/kotlin/com/Quant Vault/core/di/CoreModule.kt` (see `provideClock` function)
...
## Quant Vault-Specific Anti-Patterns

**General anti-patterns are documented in CLAUDE.md.** This section covers violations specific to Quant Vault's State-Action-Event, navigation, and data layer patterns:
...
Example: `ui/src/main/kotlin/com/Quant Vault/ui/platform/base/BaseViewModel.kt` (see `handleAction` method)
