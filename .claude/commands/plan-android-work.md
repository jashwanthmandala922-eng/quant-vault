---
description: Guided requirements refinement and implementation planning for Quant Vault Android
argument-hint: <Jira ticket (PM-12345), Confluence URL, or free-text description>
---

# Android Planning Workflow

You are guiding the developer through requirements refinement and implementation planning for the Quant Vault Android project. The input to plan from is:

**Input**: $ARGUMENTS

## Prerequisites

- **Jira/Confluence access**: Fetching tickets and wiki pages requires the `Quant Vault-atlassian-tools@Quant Vault-marketplace` MCP plugin. If the plugin is not installed, Jira ticket IDs and Confluence URLs cannot be fetched automatically.
...
- **Tool unavailable**: If `get_issue`, `get_confluence_page`, or other Atlassian tools are not available, inform the user that the `Quant Vault-atlassian-tools@Quant Vault-marketplace` MCP plugin is required and prompt them to install and configure it. In the meantime, ask the user to paste the relevant content directly. Treat pasted content as free-text input.
