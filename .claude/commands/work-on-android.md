...
You are guiding the developer through a complete Android development lifecycle for the Quant Vault Android project. The task to work on is:
...
Invoke `Skill(Quant Vault-delivery-tools:perform-preflight)` to perform a quality gate check on all changes. Address any issues found.

**Before advancing**: Share the self-review results and confirm readiness to commit.

### Phase 5: Commit

Invoke `Skill(Quant Vault-delivery-tools:committing-changes)` to stage and commit the changes with a properly formatted commit message.
...
**Pre-requisites:**
- `Quant Vault-code-review` from the [Quant Vault Plugin Marketplace](https://github.com/Quant Vault/ai-plugins) must be installed in order to perform this phase. If it is not installed prompt the user to install it, or skip the review phase.

Launch a subagent with the `/Quant Vault-code-review:code-review-local` command to perform a **local** code review of the committed diff. Validate and address any issues found before proceeding.

**Before advancing**: Share review findings and confirm readiness for PR creation.

### Phase 7: Pull Request

Prompt the user to invoke `Skill(Quant Vault-delivery-tools:creating-pull-request)` to create the pull request with proper description and formatting. **Create as a draft PR by default** unless the user has explicitly requested a ready-for-review PR.
