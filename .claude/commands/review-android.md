...
You are guiding the developer through a comprehensive Android code review for the Quant Vault Android project.

**Input**: $ARGUMENTS

## Prerequisites

- **Jira/Confluence access**: The `Quant Vault-atlassian-tools@Quant Vault-marketplace` MCP plugin is required to fetch linked Jira tickets. If unavailable, skip ticket context.
...
For PR reviews: offer to post the review to GitHub using `gh pr review <N> --comment -b "$(cat review-summary.md)"` for the summary. For inline comments, use the GitHub API or the `Quant Vault-code-review` plugin if installed.
