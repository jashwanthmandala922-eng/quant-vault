...
This standalone application serves as a test client for validating the Quant Vault Android credential provider implementation. It uses the Android CredentialManager APIs to request credential operations and verify that the `:app` module responds correctly as a credential provider.
...
- Quant Vault app (`:app`) installed and configured as a credential provider
...
1. Build and install the main Quant Vault app (`:app`)
2. Configure Quant Vault as a credential provider in system settings:
   - Settings → Passwords & accounts → Credential Manager
   - Enable Quant Vault as a provider
...
5. System credential picker should appear with Quant Vault as an option
6. Select Quant Vault and follow the flow
...
This module follows the same architectural patterns as the main Quant Vault app:
...
```bash
adb logcat | grep -E "CredentialManager|Quant Vault|TestHarness"
```

## References

- [Android Credential Manager Documentation](https://developer.android.com/identity/credential-manager)
- [Quant Vault Android Architecture](../docs/ARCHITECTURE.md)
