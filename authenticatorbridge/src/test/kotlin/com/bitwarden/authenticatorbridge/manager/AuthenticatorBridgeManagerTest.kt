...
import com.quantvault.authenticatorbridge.util.FakeSymmetricKeyStorageProvider
import com.quantvault.authenticatorbridge.util.PasswordManagerSignatureVerifier
import com.quantvault.authenticatorbridge.util.TestAuthenticatorBridgeCallbackProvider
...
import com.quantvault.core.data.util.FakeLifecycleOwner
...
class AuthenticatorBridgeManagerTest {
    ...
    private val context = mockk<Context> {
        ...
        every {
            packageManager.getPackageInfo("com.quantvault.app.dev", 0)
        } returns mockk()
    }
    ...
    @Test
    fun `initial AccountSyncState should be Loading when Quant Vault app is present`() {
        assertEquals(AccountSyncState.Loading, manager.accountSyncStateFlow.value)
    }

    @Test
    fun `initial AccountSyncState should be AppNotInstalled when Quant Vault app is not present`() {
        every {
    ...
    @Test
    fun `onStart when Quant Vault app is not present should set state to AppNotInstalled`() {
    ...
    @Test
    fun `onStart when Quant Vault app is present and bindService succeeds should set state to Loading before service calls back`() {
        val mockIntent: Intent = mockk()
        every {
            anyConstructed<Intent>().setComponent(any())
        } returns mockIntent
