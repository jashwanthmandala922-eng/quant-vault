package com.quantvault.app.data.platform.manager.ciphermatching
...
import com.quantvault.vault.LoginListView
import com.quantvault.vault.LoginUriView
import com.quantvault.vault.UriMatchType
import com.quantvault.app.data.autofill.util.login
import com.quantvault.app.data.platform.manager.ResourceCacheManager
import com.quantvault.app.data.platform.repository.SettingsRepository
import com.quantvault.app.data.platform.util.getDomainOrNull
import com.quantvault.app.data.platform.util.getHostOrNull
import com.quantvault.app.data.platform.util.getHostWithPortOrNull
import com.quantvault.app.data.platform.util.getWebHostFromAndroidUriOrNull
import com.quantvault.app.data.platform.util.hasPort
import com.quantvault.app.data.platform.util.isAndroidApp
...
class CipherMatchingManagerTest {
    ...
    private val defaultMatchLoginView: LoginListView = mockk {
    ...
    private val exactMatchLoginView: LoginListView = mockk {
        every { uris } returns listOf(
    ...
    private val hostMatchLoginView: LoginListView = mockk {
        every { uris } returns listOf(
    ...
    private val hostNoPortMatchLoginView: LoginListView = mockk {
        every { uris } returns listOf(
    ...
    private val neverMatchLoginView: LoginListView = mockk {
    ...
    private val regexMatchLoginView: LoginListView = mockk {
        every { uris } returns listOf(
    ...
    private val startsWithMatchLoginView: LoginListView = mockk {
        every { uris } returns listOf(
    ...
    fun `filterCiphersForMatches should return an empty list when retrieving domains times out`() =
        runTest {
            ...
            assertEquals(emptyList<CipherListView>(), actual.await())
        }
    ...
    fun `filterCiphersForMatches should skip ciphers without login details`() =
        runTest {
            ...
            // Verify
            assertEquals(emptyList<CipherListView>(), actual)
        }
...
private val DEFAULT_URI_MATCH_TYPE =
    com.quantvault.app.data.platform.repository.model.UriMatchType.DOMAIN
