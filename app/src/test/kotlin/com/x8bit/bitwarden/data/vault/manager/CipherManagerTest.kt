...
import com.quantvault.data.manager.model.DownloadResult
...
import com.quantvault.network.model.createMockAttachment
import com.quantvault.network.model.createMockAttachmentResponse
import com.quantvault.network.model.createMockCipher
import com.quantvault.network.model.createMockCipherJsonRequest
import com.quantvault.network.model.createMockCollection
import com.quantvault.network.model.createMockLogin
...
import com.quantvault.app.data.platform.manager.model.SyncCipherDeleteData
import com.quantvault.app.data.platform.manager.model.SyncCipherUpsertData
...
import com.quantvault.app.data.vault.datasource.sdk.model.createMockEncryptionContext
import com.quantvault.app.data.vault.datasource.sdk.model.createMockSdkAttachment
...
import com.quantvault.app.data.vault.repository.model.ShareCipherResult
...
import com.quantvault.app.data.vault.repository.util.toEncryptedNetworkCipher
import com.quantvault.app.data.vault.repository.util.toEncryptedNetworkCipherResponse
import com.quantvault.app.data.vault.repository.util.toEncryptedSdkCipher
import com.quantvault.app.data.vault.repository.util.toNetworkAttachmentRequest
...
class CipherManagerTest {
    ...
    fun `downloadAttachment with failed download should return Failure`() = runTest {
        ...
        val response = mockk<SyncResponseJson.Cipher.Attachment> {
            every { url } returns "https://Quant Vault.com"
        }
        coEvery {
            ciphersService.getCipherAttachment(cipherId = any(), attachmentId = any())
        } returns response.asSuccess()
        ...
        coVerify(exactly = 1) {
            ...
            fileManager.downloadFileToCache("https://Quant Vault.com")
        }
    }
    ...
    fun `downloadAttachment with successful decryption should delete file and return Success`() =
        runTest {
            ...
            coVerify(exactly = 1) {
                ...
                fileManager.downloadFileToCache(url = "https://Quant Vault.com")
                vaultSdkSource.decryptFile(
                    userId = MOCK_USER_STATE.activeUserId,
                    cipher = mockCipher,
                    attachmentView = attachmentView,
