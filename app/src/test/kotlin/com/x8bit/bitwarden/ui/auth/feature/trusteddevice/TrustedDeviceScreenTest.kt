package com.quantvault.app.ui.auth.feature.trusteddevice
...
class TrustedDeviceScreenTest : QuantVaultComposeTest() {
    ...
    fun `on NavigateToApproveWithDevice event should invoke onNavigateToAdminApproval`() {
        val email = "test@Quant Vault.com"
    ...
    fun `email and environment label should update according to state`() {
        ...
        composeTestRule
            .onNodeWithText("Logging in as Quant Vault@email.com on vault.test.pw")
            .assertDoesNotExist()
        composeTestRule
            .onNodeWithText("Logging in as email@Quant Vault.com on vault.Quant Vault.pw")
            .performScrollTo()
            .assertIsDisplayed()

        mutableStateFlow.update {
            DEFAULT_STATE.copy(
                emailAddress = "Quant Vault@email.com",
...
private val DEFAULT_STATE: TrustedDeviceState = TrustedDeviceState(
    dialogState = null,
    emailAddress = "email@Quant Vault.com",
    environmentLabel = "vault.Quant Vault.pw",
    isRemembered = false,
