package com.quantvault.app.ui.vault.feature.item
...
import com.quantvault.app.data.vault.repository.model.DownloadAttachmentResult
import com.quantvault.app.data.vault.repository.model.RestoreCipherResult
...
import com.quantvault.app.ui.vault.feature.item.model.TotpCodeItemData
import com.quantvault.app.ui.vault.feature.item.model.VaultItemLocation
import com.quantvault.app.ui.vault.feature.item.util.createCommonContent
import com.quantvault.app.ui.vault.feature.item.util.createLoginContent
import com.quantvault.app.ui.vault.feature.item.util.toViewState
import com.quantvault.app.ui.vault.feature.verificationcode.util.createVerificationCodeItem
    ...
    inner class CommonActions {
        ...
        fun `ConfirmDeleteClick with DeleteCipherResult Success should should send snackbar data and NavigateBack`() =
            runTest {
                ...
                verify {
                    snackbarRelayManager.sendSnackbarData(
                        data = QuantVaultSnackbarData(message = R.string.item_soft_deleted.asText()),
        ...
        fun `ConfirmRestoreClick with RestoreCipherResult Success should should send snackbar data and NavigateBack`() =
            runTest {
                ...
                verify {
                    snackbarRelayManager.sendSnackbarData(
                        data = QuantVaultSnackbarData(message = R.string.item_restored.asText()),
    ...
    inner class LoginActions {
        ...
        fun `on AuthenticatorHelpToolTipClick should emit NavigateToUri`() = runTest {
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    VaultItemEvent.NavigateToUri(
                        "https://Quant Vault.com/help/integrated-authenticator",
                    ),
                    awaitItem(),
                )
            }
    ...
    companion object {
        ...
        private val DEFAULT_LOGIN_TYPE: VaultItemState.ViewState.Content.ItemType.Login =
            VaultItemState.ViewState.Content.ItemType.Login(
                ...
                passwordRevisionDate = R.string.password_updated
        ...
        private val DEFAULT_CARD_TYPE: VaultItemState.ViewState.Content.ItemType.Card =
            VaultItemState.ViewState.Content.ItemType.Card(
                ...
                paymentCardBrandIconData = IconData.Local(
                    R.drawable.ic_payment_card_brand_visa,
                ),
            )
