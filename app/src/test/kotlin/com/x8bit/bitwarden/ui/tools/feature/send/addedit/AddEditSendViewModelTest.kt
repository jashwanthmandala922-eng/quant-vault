package com.quantvault.app.ui.tools.feature.send.addedit
...
import com.quantvault.app.data.auth.repository.model.PolicyInformation
...
import com.quantvault.app.data.vault.repository.model.CreateSendResult
...
import com.quantvault.app.data.vault.repository.model.UpdateSendResult
...
import com.quantvault.app.ui.tools.feature.send.addedit.model.AddEditSendType
import com.quantvault.app.ui.tools.feature.send.addedit.model.AuthEmail
import com.quantvault.app.ui.tools.feature.send.addedit.model.SendAuth
import com.quantvault.app.ui.tools.feature.send.addedit.util.toSendView
import com.quantvault.app.ui.tools.feature.send.addedit.util.toViewState
import com.quantvault.app.ui.tools.feature.send.model.SendItemType
import com.quantvault.app.ui.tools.feature.send.util.toSendUrl
    ...
    fun `SaveClick with EMAIL auth type and blank emails should show error dialog`() {
        ...
        assertEquals(
            DEFAULT_STATE.copy(
                viewState = viewState,
                dialogState = AddEditSendState.DialogState.Error(
                    title = R.string.no_email_addresses_entered.asText(),
                    message = R.string.enter_at_least_one_valid_email_address_to_share_this_send
                        .asText(),
                ),
            ),
            viewModel.stateFlow.value,
    ...
    fun `SaveClick with EMAIL auth type and valid and invalid emails should show error dialog`() {
        ...
        assertEquals(
            DEFAULT_STATE.copy(
                viewState = viewState,
                dialogState = AddEditSendState.DialogState.Error(
                    title = R.string.invalid_email_addresses.asText(),
                    message = R.string.one_or_more_email_addresses_are_incorrect.asText(),
                ),
            ),
            viewModel.stateFlow.value,
        )
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    AddEditSendEvent.ShowSnackbar(
                        data = QuantVaultSnackbarData(message = R.string.password_removed.asText()),
                    ),
                    awaitItem(),
                )
            }
        ...
        runTest {
            ...
            verify(exactly = 1) {
                snackbarRelayManager.sendSnackbarData(
                    data = QuantVaultSnackbarData(message = R.string.send_deleted.asText()),
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    AddEditSendEvent.NavigateToPremium(
                        uri = "https://vault.Quant Vault.com/#/settings/subscription/premium?callToAction=upgradeToPremium",
                    ),
                    awaitItem(),
                )
            }
...
private const val DEFAULT_ENVIRONMENT_URL = "https://send.Quant Vault.com/#"
