...
import com.quantvault.app.ui.platform.manager.biometrics.BiometricSupportStatus
...
@Suppress("LargeClass")
class AccountSecurityScreenTest : QuantVaultComposeTest() {
        ...
        composeTestRule
            .onAllNodesWithText(
                text = "Your PIN must be at least 4 characters. Your PIN settings will be reset " +
                    "if you manually log out of the Quant Vault app.",
            )
            .filterToOne(hasAnyAncestor(isDialog()))
            .assertIsDisplayed()
        composeTestRule
    ...
    @Test
    fun `sync with Quant Vault authenticator UI should be displayed according to state`() {
    ...
    @Test
    fun `sync with Quant Vault authenticator click should send AuthenticatorSyncToggle action`() {
        mutableStateFlow.update { DEFAULT_STATE.copy(shouldShowEnableAuthenticatorSync = true) }
        composeTestRule
