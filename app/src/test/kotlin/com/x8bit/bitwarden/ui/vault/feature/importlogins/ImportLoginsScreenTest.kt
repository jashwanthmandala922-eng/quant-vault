package com.quantvault.app.ui.vault.feature.importlogins
...
class ImportLoginsScreenTest : QuantVaultComposeTest() {
    ...
    fun `OpenHelpLink event is used to open URI with intent manager`() {
        ...
        verify {
            intentManager.startCustomTabsActivity("https://Quant Vault.com/help/import-data/".toUri())
        }
    }
    ...
    fun `Step two content shows correct vault url when vault url is set`() {
        val url = "vault.Quant Vault.com.testing"
        mutableImportLoginsStateFlow.update {
            it.copy(
                viewState = ImportLoginsState.ViewState.ImportStepTwo,
