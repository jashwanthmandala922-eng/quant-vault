...
import com.quantvault.network.interceptor.FakeInterceptorChain
...
class GlideCookieInterceptorTest {
    ...
    fun `intercept should proceed without cookie header when no cookies available`() {
        val originalRequest = Request.Builder()
            .url("https://vault.Quant Vault.com/icons/icon.png")
            .build()
        val chain = FakeInterceptorChain(originalRequest)

        every {
            mockCookieProvider.getCookies("vault.Quant Vault.com")
        } returns emptyList()
    ...
    fun `intercept should throw CookieRedirectException on 302 response without cookies`() {
        ...
        assertEquals("vault.Quant Vault.com", exception.hostname)
    }
