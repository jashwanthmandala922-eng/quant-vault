package com.quantvault.app.ui.auth.feature.twofactorlogin
...
import com.quantvault.app.data.auth.repository.util.generateUriForWebAuth
...
import com.quantvault.app.data.platform.util.webAuthnAuthTabData
...
class TwoFactorLoginViewModelTest : BaseViewModelTest() {
        ...
        runTest {
            val authMethodsData = mapOf(
                TwoFactorAuthMethod.DUO to JsonObject(
                    mapOf("AuthUrl" to JsonPrimitive("Quant Vault.com")),
                ),
            )
            ...
            every { "Quant Vault.com".toUri() } returns mockkUri
            val viewModel = createViewModel(
                state = DEFAULT_STATE.copy(
                    authMethod = TwoFactorAuthMethod.DUO,
                ),
    ...
    fun `ContinueButtonClick login should emit NavigateToWebAuth when auth method is WEB_AUTH and data is non-null`() =
        runTest {
            val data = JsonObject(mapOf("AuthUrl" to JsonPrimitive("Quant Vault.com")))
            val response = GetTokenResponseJson.TwoFactorRequired(
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    TwoFactorLoginEvent.ShowSnackbar(
                        message = R.string.there_was_an_error_starting_web_authn_two_factor_authentication
                            .asText(),
                    ),
                    awaitItem(),
                )
            ...
            viewModel.stateFlow.test {
                ...
                assertEquals(
                    DEFAULT_STATE.copy(
                        dialogState = TwoFactorLoginState.DialogState.Error(
                            title = R.string.an_error_has_occurred.asText(),
                            message = R.string.this_is_not_a_recognized_Quant Vault_server_you_may_need_to_check_with_your_provider_or_update_your_server.asText(),
                        ),
                    ),
                    awaitItem(),
                )
    ...
    fun `SelectAuthMethod with RECOVERY_CODE should launch the NavigateToRecoveryCode event`() =
        runTest {
            val mockkUri = mockk<Uri>()
            every {
                Uri.parse("https://vault.Quant Vault.com/#/recover-2fa")
            } returns mockkUri
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    TwoFactorLoginEvent.NavigateToRecoveryCode(
                        uri = "https://vault.Quant Vault.com/#/recover-2fa".toUri(),
                    ),
                    awaitItem(),
                )
            }
