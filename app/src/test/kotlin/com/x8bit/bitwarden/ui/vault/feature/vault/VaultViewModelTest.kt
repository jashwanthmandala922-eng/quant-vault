package com.quantvault.app.ui.vault.feature.vault
...
import com.quantvault.app.data.autofill.manager.browser.BrowserAutofillDialogManager
import com.quantvault.app.data.billing.manager.PremiumStateManager
import com.quantvault.app.data.platform.manager.CredentialExchangeRegistryManager
...
import com.quantvault.app.data.platform.manager.ReviewPromptManager
...
import com.quantvault.app.data.vault.datasource.sdk.model.createMockCardListView
...
import com.quantvault.app.ui.vault.feature.vault.util.toSnackbarData
import com.quantvault.app.ui.vault.feature.vault.util.toViewState
...
class VaultViewModelTest : BaseViewModelTest() {
            ...
            assertEquals(
                DEFAULT_STATE.copy(
                    dialog = VaultState.DialogState.VaultLoadKdfUpdateRequired(
                        title = R.string.update_your_encryption_settings.asText(),
                        message = R.string.the_new_recommended_encryption_settings_will_improve_your_account_desc_long
                            .asText(),
                    ),
                ),
                viewModel.stateFlow.value,
    ...
    fun `ArchiveClick with ArchiveCipherResult Success should emit a ShowSnackbar event`() =
        runTest {
            ...
            viewModel.eventFlow.test {
                assertEquals(
                    VaultEvent.ShowSnackbar(R.string.item_moved_to_archived.asText()),
                    awaitItem(),
                )
            }
        }
    ...
    fun `UnarchiveClick with UnarchiveCipherResult Success should emit a ShowSnackbar event`() =
        runTest {
            ...
            viewModel.eventFlow.test {
                assertEquals(
                    VaultEvent.ShowSnackbar(R.string.item_moved_to_vault.asText()),
                    awaitItem(),
                )
            }
        }
            ...
            assertEquals(
                DEFAULT_STATE.copy(
                    ...
                    viewState = VaultState.ViewState.Content(
                        ...
                        archiveSubText = R.string.premium_subscription_required.asText(),
                        archiveEndIcon = R.drawable.ic_locked,
                        showCardGroup = true,
                    ),
                ),
                viewModel.stateFlow.value,
    ...
    fun `when PREMIUM_UPGRADED relay emits, snackbar is shown`() =
        runTest {
            val viewModel = createViewModel()
            val expectedSnackbarData = QuantVaultSnackbarData(
    ...
    fun `UpdatedKdfToMinimumsReceived with Success should clear dialog and send a ShowSnackbar event`() =
        runTest {
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    VaultEvent.ShowSnackbar(R.string.encryption_settings_updated.asText()),
                    awaitItem(),
                )
            }
        }
        ...
        runTest {
            ...
            assertEquals(
                DEFAULT_STATE.copy(
                    dialog = VaultState.DialogState.Error(
                        title = R.string.an_error_has_occurred.asText(),
                        message = R.string.kdf_update_failed_active_account_not_found
                            .asText(),
                    ),
                ),
                viewModel.stateFlow.value,
    ...
    fun `UpdatedKdfToMinimumsReceived with Error should show error dialog`() = runTest {
        ...
        assertEquals(
            DEFAULT_STATE.copy(
                dialog = VaultState.DialogState.Error(
                    title = R.string.an_error_has_occurred.asText(),
                    message = R.string.an_error_occurred_while_trying_to_update_your_kdf_settings
                        .asText(),
                    error = testError,
                ),
            ),
        ...
        runTest {
            ...
            assertEquals(
                createMockVaultState(
                    ...
                    dialog = VaultState.DialogState.VaultLoadKdfUpdateRequired(
                        title = R.string.update_your_encryption_settings.asText(),
                        message = R.string.the_new_recommended_encryption_settings_will_improve_your_account_desc_long.asText(),
                    ),
                ),
                viewModel.stateFlow.value,
            )
