...
class StartRegistrationScreenTest : QuantVaultComposeTest() {
    ...
    fun `NavigateToTerms event should invoke intent manager`() {
        ...
        verify {
            intentManager.launchUri("https://Quant Vault.com/terms/".toUri())
        }
    }
    ...
    fun `when NavigateToServerSelectionInfo is observed event should invoke intent manager`() {
        ...
        verify {
            intentManager.launchUri(
                uri = "https://Quant Vault.com/help/server-geographies/".toUri(),
            )
        }
    }
    ...
    fun `when unsubscribe custom action invoked should send UnsubscribeMarketingEmailsClick`() {
        composeTestRule
            .onNodeWithText(
                text = "Get emails from Quant Vault for announcements, advice, and " +
