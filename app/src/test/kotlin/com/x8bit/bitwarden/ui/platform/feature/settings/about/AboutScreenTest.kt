package com.quantvault.app.ui.platform.feature.settings.about
...
class AboutScreenTest : QuantVaultComposeTest() {
    ...
    @Test
    fun `on Quant Vault help center click should display confirmation dialog and confirm click should emit HelpCenterClick`() {
        composeTestRule.onNode(isDialog()).assertDoesNotExist()
        composeTestRule.onNodeWithText("Quant Vault help center").performClick()
    ...
    @Suppress("MaxLineLength")
    @Test
    fun `on Quant Vault web vault click should display confirmation dialog and confirm click should emit WebVaultClick`() {
        composeTestRule.onNode(isDialog()).assertDoesNotExist()
        composeTestRule.onNodeWithText("Quant Vault web vault").performClick()
        composeTestRule.onNode(isDialog()).assertExists()
        composeTestRule
            .onAllNodesWithText("Continue")
            .filterToOne(hasAnyAncestor(isDialog()))
    ...
    fun `copyright info should update according to the state`() = runTest {
        ...
        mutableStateFlow.update {
            it.copy(copyrightInfo = "© Quant Vault Inc. 2015-$currentYear".asText())
        }

        composeTestRule.onNodeWithText("© Quant Vault Inc. 2015-$currentYear")
            .performScrollTo()
            .assertIsDisplayed()
    }
