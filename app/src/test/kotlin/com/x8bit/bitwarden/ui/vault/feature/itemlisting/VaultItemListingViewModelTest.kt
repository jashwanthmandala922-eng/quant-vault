package com.quantvault.app.ui.vault.feature.itemlisting
...
import com.quantvault.data.repository.util.baseIconUrl
import com.quantvault.data.repository.util.baseWebSendUrl
...
import com.quantvault.vault.CipherListViewType
...
import com.quantvault.app.data.auth.repository.model.LogoutReason
import com.quantvault.app.data.auth.repository.model.SwitchAccountResult
import com.quantvault.app.data.auth.repository.model.UserState
import com.quantvault.app.data.auth.repository.model.ValidatePasswordResult
import com.quantvault.app.data.auth.repository.model.ValidatePinResult
import com.quantvault.app.data.auth.repository.model.VaultUnlockType
import com.quantvault.app.data.autofill.accessibility.manager.AccessibilitySelectionManager
import com.quantvault.app.data.autofill.accessibility.manager.AccessibilitySelectionManagerImpl
import com.quantvault.app.data.autofill.manager.AutofillSelectionManager
import com.quantvault.app.data.autofill.manager.AutofillSelectionManagerImpl
import com.quantvault.app.data.autofill.model.AutofillSaveItem
import com.quantvault.app.data.autofill.model.AutofillSelectionData
import com.quantvault.app.data.billing.manager.PremiumStateManager
import com.quantvault.app.data.credentials.manager.QuantVaultCredentialManager
import com.quantvault.app.data.credentials.manager.OriginManager
import com.quantvault.app.data.credentials.model.CreateCredentialRequest
import com.quantvault.app.data.credentials.model.Fido2CredentialAssertionResult
import com.quantvault.app.data.credentials.model.Fido2RegisterCredentialResult
import com.quantvault.app.data.credentials.model.UserVerificationRequirement
import com.quantvault.app.data.credentials.model.ValidateOriginResult
import com.quantvault.app.data.credentials.model.createMockCreateCredentialRequest
import com.quantvault.app.data.credentials.model.createMockFido2CredentialAssertionRequest
import com.quantvault.app.data.credentials.model.createMockGetCredentialsRequest
import com.quantvault.app.data.credentials.model.createMockProviderGetPasswordCredentialRequest
import com.quantvault.app.data.credentials.parser.RelyingPartyParser
import com.quantvault.app.data.credentials.repository.PrivilegedAppRepository
...
import com.quantvault.app.data.platform.manager.SpecialCircumstanceManagerImpl
import com.quantvault.app.data.platform.manager.ciphermatching.CipherMatchingManager
import com.quantvault.app.data.platform.manager.clipboard.QuantVaultClipboardManager
import com.quantvault.app.data.platform.manager.event.OrganizationEventManager
import com.quantvault.app.data.platform.manager.model.FirstTimeState
import com.quantvault.app.data.platform.manager.model.OrganizationEvent
import com.quantvault.app.data.platform.manager.model.SpecialCircumstance
import com.quantvault.app.data.platform.manager.network.NetworkConnectionManager
...
import com.quantvault.app.data.platform.util.getSignatureFingerprintAsHexString
import com.quantvault.app.data.vault.datasource.sdk.model.createMockCardView
import com.quantvault.app.data.vault.datasource.sdk.model.createMockCipherListView
...
import com.quantvault.app.data.vault.datasource.sdk.model.createMockDecryptCipherListResult
import com.quantvault.app.data.vault.datasource.sdk.model.createMockFolderView
import com.quantvault.app.data.vault.datasource.sdk.model.createMockLoginListView
import com.quantvault.app.data.vault.datasource.sdk.model.createMockSdkFido2CredentialList
import com.quantvault.app.data.vault.datasource.sdk.model.createMockSendView
import com.quantvault.app.data.vault.manager.model.GetCipherResult
import com.quantvault.app.data.vault.repository.VaultRepository
import com.quantvault.app.data.vault.repository.model.ArchiveCipherResult
import com.quantvault.app.data.vault.repository.model.DeleteSendResult
import com.quantvault.app.data.vault.repository.model.GenerateTotpResult
import com.quantvault.app.data.vault.repository.model.RemovePasswordSendResult
import com.quantvault.app.data.vault.repository.model.UnarchiveCipherResult
import com.quantvault.app.data.vault.repository.model.VaultData
import com.quantvault.app.ui.credentials.manager.model.AssertFido2CredentialResult
import com.quantvault.app.ui.credentials.manager.model.CreateCredentialResult
import com.quantvault.app.ui.credentials.manager.model.GetCredentialsResult
import com.quantvault.app.ui.credentials.manager.model.GetPasswordCredentialResult
import com.quantvault.app.ui.platform.feature.search.model.SearchType
import com.quantvault.app.ui.platform.model.SnackbarRelay
import com.quantvault.app.ui.tools.feature.send.model.SendItemType
import com.quantvault.app.ui.vault.components.model.CreateVaultItemType
import com.quantvault.app.ui.vault.feature.addedit.util.createMockPasskeyAttestationOptions
import com.quantvault.app.ui.vault.feature.itemlisting.model.ListingItemOverflowAction
import com.quantvault.app.ui.vault.feature.itemlisting.util.createMockDisplayItemForCipher
import com.quantvault.app.ui.vault.feature.vault.model.VaultFilterType
import com.quantvault.app.ui.vault.feature.vault.util.toAccountSummaries
import com.quantvault.app.ui.vault.feature.vault.util.toActiveAccountSummary
...
class VaultItemListingViewModelTest : BaseViewModelTest() {
    ...
    private val QuantVaultCredentialManager: QuantVaultCredentialManager = mockk {
    ...
    private val mutableSnackbarDataFlow: MutableSharedFlow<QuantVaultSnackbarData> =
    ...
    fun `ItemClick for vault item during FIDO 2 registration should show overwrite passkey confirmation when selected cipher has existing passkey`() {
        runTest {
            ...
            coEvery {
                QuantVaultCredentialManager.registerFido2Credential(
                    userId = any(),
                    callingAppInfo = any(),
    ...
    fun `ItemClick for vault item during FIDO 2 registration should show loading dialog, then request user verification when required`() =
        runTest {
            ...
            every {
                QuantVaultCredentialManager.getUserVerificationRequirement(
    ...
    fun `ItemClick for vault item during FIDO 2 registration should skip user verification and perform registration when discouraged`() =
        runTest {
            ...
            every {
                QuantVaultCredentialManager.getPasskeyAttestationOptionsOrNull(any())
    ...
    fun `ItemClick for vault item during FIDO 2 registration should skip user verification when user is verified`() =
        runTest {
            ...
            every { QuantVaultCredentialManager.isUserVerified } returns true
            coEvery {
                QuantVaultCredentialManager.registerFido2Credential(
                    userId = any(),
                    callingAppInfo = any(),
            ...
            coVerify { QuantVaultCredentialManager.isUserVerified }
            coVerify(exactly = 1) {
                QuantVaultCredentialManager.registerFido2Credential(
                    userId = DEFAULT_USER_STATE.activeUserId,
                    createPublicKeyCredentialRequest = any(),
        ...
        assertEquals(
            createVaultItemListingState().copy(
                dialogState = VaultItemListingState.DialogState.CipherDecryptionError(
                    ...
                    message = R.string.Quant Vault_could_not_decrypt_this_vault_item_description_long.asText(),
            ...
            viewModel.eventFlow.test {
                ...
                assertEquals(
                    VaultItemListingEvent.NavigateToUrl(
                        url = "https://vault.Quant Vault.com/#/" +
    ...
    fun `ArchiveClick without Premium should show ArchiveRequiresPremium dialog`() = runTest {
        ...
        viewModel.trySendAction(
            VaultItemListingsAction.OverflowOptionClick(
                action = ListingItemOverflowAction.VaultAction.ArchiveClick(cipherId = "mockId-1"),
            ),
        )
        ...
        assertEquals(
            createVaultItemListingState(
                isPremium = false,
                dialogState = VaultItemListingState.DialogState.ArchiveRequiresPremium,
            ),
            viewModel.stateFlow.value,
        )
    }
    ...
    fun `ArchiveClick with ArchiveCipherResult Success should emit a ShowSnackbar event`() =
        runTest {
            ...
            viewModel.trySendAction(
                VaultItemListingsAction.OverflowOptionClick(
                    action = ListingItemOverflowAction.VaultAction.ArchiveClick(
                        cipherId = "mockId-1",
                    ),
                ),
            )
            ...
            viewModel.eventFlow.test {
                assertEquals(
                    VaultItemListingEvent.ShowSnackbar(
                        message = R.string.item_moved_to_archived.asText(),
                    ),
                    awaitItem(),
                )
            }
    ...
    fun `ArchiveClick with ArchiveCipherResult Failure should show generic error`() = runTest {
        ...
        assertEquals(
            createVaultItemListingState(
                dialogState = VaultItemListingState.DialogState.Error(
                    title = R.string.an_error_has_occurred.asText(),
                    message = R.string.unable_to_archive_selected_item.asText(),
                    throwable = error,
                ),
            ),
            viewModel.stateFlow.value,
    ...
    fun `UnarchiveClick with UnarchiveCipherResult Success should emit a ShowSnackbar event`() =
        runTest {
            ...
            viewModel.eventFlow.test {
                assertEquals(
                    VaultItemListingEvent.ShowSnackbar(
                        message = R.string.item_moved_to_vault.asText(),
                    ),
                    awaitItem(),
                )
            }
    ...
    fun `UnarchiveClick with UnarchiveCipherResult Failure should show generic error`() = runTest {
        ...
        assertEquals(
            createVaultItemListingState(
                dialogState = VaultItemListingState.DialogState.Error(
                    title = R.string.an_error_has_occurred.asText(),
                    message = R.string.unable_to_unarchive_selected_item.asText(),
                    throwable = error,
                ),
            ),
            viewModel.stateFlow.value,
                        ...
                        displayItemList = listOf(
                            createMockDisplayItemForCipher(
                                ...
                                iconData = IconData.Network(
                                    uri = "https://icons.Quant Vault.net/www.mockuri.com/icon.png",
    ...
    fun `Fido2RegisterCredentialResult Success should show toast and emit CompleteCredentialRegistration result`() =
        runTest {
            ...
            verify {
                ...
                QuantVaultCredentialManager.isUserVerified = false
            }
        }
    ...
    fun `GetCredentialsRequest should validate request and emit CompleteProviderGetCredentialsRequest event`() =
        runTest {
            ...
            coEvery {
                QuantVaultCredentialManager.getCredentialEntries(any())
    ...
    fun `Fido2AssertionRequest should display loading dialog then request user verification when user is not verified and verification is REQUIRED`() =
        runTest {
            ...
            viewModel.eventFlow.test {
                ...
                verify { QuantVaultCredentialManager.isUserVerified }
                assertEquals(
                    VaultItemListingEvent.CredentialManagerUserVerification(
                        isRequired = true,
    ...
    fun `Fido2AssertionRequest should skip user verification when user is not verified and verification is DISCOURAGED`() =
        runTest {
            ...
            coEvery {
                QuantVaultCredentialManager.authenticateFido2Credential(
                    userId = "activeUserId",
                    request = any(),
                    selectedCipherView = mockCipherView,
                    callingAppInfo = any(),
            ...
            coVerify {
                QuantVaultCredentialManager.isUserVerified
                QuantVaultCredentialManager.authenticateFido2Credential(
                    userId = "activeUserId",
                    request = any(),
                    selectedCipherView = mockCipherView,
        ...
        runTest {
            ...
            viewModel.stateFlow.test {
                assertEquals(
                    VaultItemListingState.DialogState.CredentialManagerOperationFail(
                        title = R.string.an_error_has_occurred.asText(),
                        message = R.string.passkey_operation_failed_because_relying_party_cannot_be_identified
                            .asText(),
                    ),
    ...
    fun `UserVerificationLockout should display CredentialManagerOperationFail and set isUserVerified to false`() {
        ...
        verify { QuantVaultCredentialManager.isUserVerified = false }
        assertEquals(
            VaultItemListingState.DialogState.CredentialManagerOperationFail(
                title = R.string.an_error_has_occurred.asText(),
                message = R.string.credential_operation_failed_because_user_is_locked_out
                    .asText(),
            ),
            viewModel.stateFlow.value.dialogState,
        )
    ...
    fun `UserVerificationSuccess should set isUserVerified to true, and register FIDO 2 credential when verification result is received`() =
        runTest {
            ...
            coVerify {
                QuantVaultCredentialManager.isUserVerified = true
                QuantVaultCredentialManager.registerFido2Credential(
    ...
    fun `MasterPasswordUserVerificationSubmit should display CredentialManagerOperationFail when user has no retries remaining`() {
        ...
        val selectedCipherId = "selectedCipherId"
        val password = "password"
        every { QuantVaultCredentialManager.hasAuthenticationAttemptsRemaining() } returns false
        coEvery {
            authRepository.validatePassword(password = password)
        } returns ValidatePasswordResult.Success(isValid = false)
        ...
        assertEquals(
            VaultItemListingState.DialogState.CredentialManagerOperationFail(
                title = R.string.an_error_has_occurred.asText(),
                message = R.string.credential_operation_failed_because_user_verification_attempts_exceeded
                    .asText(),
            ),
            viewModel.stateFlow.value.dialogState,
        )
                ...
                assertEquals(
                    VaultItemListingEvent.CompleteProviderGetPasswordCredentialRequest(
                        result = GetPasswordCredentialResult.Error(
                            message = R.string.password_operation_failed_because_the_selected_item_does_not_have_a_valid_password
                                .asText(),
                        ),
                    ),
                    awaitItem(),
    ...
    fun `TrustPrivilegedAppClick during BeginGetCredentials should clear dialog, trust privileged app, and get credential entries`() =
        runTest {
            ...
            coVerify {
                ...
                QuantVaultCredentialManager.getCredentialEntries(
    ...
    fun `SnackbarDataReceive should update emit ShowSnackbar`() = runTest {
        val viewModel = createVaultItemListingViewModel()
        val snackbarData = QuantVaultSnackbarData(message = "Test".asText())
        viewModel.eventFlow.test {
    ...
    private fun createVaultItemListingViewModel(
        ...
        VaultItemListingViewModel(
            ...
            QuantVaultCredentialManager = QuantVaultCredentialManager,
...
private const val DEFAULT_RELYING_PARTY_ID = "www.Quant Vault.com"
