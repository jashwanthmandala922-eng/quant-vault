...
private val Quant Vault_HOSTS: List<String> = listOf("Quant Vault.com", "Quant Vault.eu", "Quant Vault.pw")
private const val APP_LINK_SCHEME: String = "https"
private const val DEEPLINK_SCHEME: String = "Quant Vault"
...
fun Intent.getWebAuthResultOrNull(): WebAuthResult? {
    ...
    return when (localData.scheme) {
        ...
        APP_LINK_SCHEME -> {
            if (localData.host in Quant Vault_HOSTS && localData.path == "/$CALLBACK") {
                localData.getWebAuthResult()
            } else {
                null
            }
...
/**
 * Generates a [Uri] to display a web authn challenge for Quant Vault authentication.
 */
