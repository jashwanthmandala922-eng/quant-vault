...
import com.quantvault.core.data.repository.util.combineDataStates
import com.quantvault.core.data.repository.util.mapNullable
...
class VaultItemViewModel @Inject constructor(
    ...
    private fun handleAttachmentDownloadClick(
        ...
        onContent { content ->
            updateDialogState(
                VaultItemState.DialogState.Loading(R.string.downloading.asText()),
            )
    ...
    private fun handleAuthenticatorHelpToolTipClick() {
        sendEvent(
            event = VaultItemEvent.NavigateToUri(
                uri = "https://Quant Vault.com/help/integrated-authenticator",
            ),
        )
    }
            ...
            is BreachCountResult.Success -> {
                VaultItemState.DialogState.Generic(
                    message = if (result.breachCount > 0) {
                        R.plurals.password_exposed.asPluralsText(
