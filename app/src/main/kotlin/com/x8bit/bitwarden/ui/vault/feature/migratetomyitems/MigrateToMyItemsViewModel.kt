...
import com.quantvault.network.util.isNoConnectionError
import com.quantvault.network.util.isTimeoutError
        ...
        mutableStateFlow.update {
            it.copy(
                dialog = MigrateToMyItemsState.DialogState.Loading(
                    message = R.string.migrating_items_to_x.asText(
                        it.organizationName,
                    ),
                ),
            )
    ...
    private fun handleHelpLinkClicked() {
        sendEvent(
            MigrateToMyItemsEvent.LaunchUri(
                uri = "https://Quant Vault.com/help/transfer-ownership/",
            ),
        )
    }
            ...
            is MigratePersonalVaultResult.Success -> {
                snackbarRelayManager.sendSnackbarData(
                    ...
                    data = QuantVaultSnackbarData(
                        message = R.string.items_transferred.asText(),
                    ),
                )
                    ...
                    it.copy(
                        dialog = if (isNetworkOrTimeoutError) {
                            MigrateToMyItemsState.DialogState.NoNetwork(
                                ...
                                message = R.string.internet_connection_required_message
                                    .asText(),
