...
fun VaultData.toViewState(
    ...
    return if (folderList.isNotEmpty() || allFilteredCipherViewList.isNotEmpty() ||
        ...
        VaultItemListingState.ViewState.NoItems(
            header = totpData
                ...
                ?: run {
                    when (itemListingType) {
                        ...
                        VaultItemListingState.ItemListingType.Vault.Archive -> {
                            R.string.no_archives_title.asText()
                        }
                    }
                },
            message = message,
            ...
            vectorRes = totpData
                ?.let { R.drawable.ill_folder_question }
                ?: run {
                    when (itemListingType) {
                        is VaultItemListingState.ItemListingType.Vault.Folder,
                        is VaultItemListingState.ItemListingType.Vault.Collection,
...
private fun CipherListView.toDisplayItemDecryptionError(): VaultItemListingState.DisplayItem =
    VaultItemListingState.DisplayItem(
        id = id.orEmpty(),
        title = R.string.error_cannot_decrypt.asText(),
        titleTestTag = "CipherNameLabel",
        secondSubtitle = null,
...
private val CipherListViewType.iconRes: Int
    get() = when (this) {
        is CipherListViewType.Login -> R.drawable.ic_globe
        CipherListViewType.SecureNote -> R.drawable.ic_note
        is CipherListViewType.Card -> R.drawable.ic_payment_card
        CipherListViewType.Identity -> R.drawable.ic_id_card
        CipherListViewType.SshKey -> R.drawable.ic_ssh_key
