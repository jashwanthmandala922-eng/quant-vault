...
import com.quantvault.core.data.repository.util.takeUntilLoaded
...
class VaultAddEditViewModel @Inject constructor(
    ...
    initialState = savedStateHandle[KEY_STATE]
        ?: run {
            ...
            val fido2AttestationOptions = providerCreateCredentialRequest?.requestJson
                ?.let { request ->
                    QuantVaultCredentialManager.getPasskeyAttestationOptionsOrNull(request)
                }
    ...
    private fun handleArchiveClick() {
        ...
        onEdit {
            mutableStateFlow.update {
                it.copy(
                    dialog = VaultAddEditState.DialogState.Loading(
                        label = R.string.archiving.asText(),
                    ),
                )
            }
        }
    ...
    private fun handleUnarchiveClick() {
        onEdit {
            mutableStateFlow.update {
                it.copy(
                    dialog = VaultAddEditState.DialogState.Loading(
                        label = R.string.unarchiving.asText(),
                    ),
                )
            }
        }
    ...
    private fun handleLoginClearFido2Credential() {
        ...
        sendEvent(event = VaultAddEditEvent.ShowSnackbar(R.string.passkey_removed.asText()))
    }
    ...
    private fun handleFido2RegisterCredentialResultReceive(
        ...
        when (action.result) {
            is Fido2RegisterCredentialResult.Error -> {
                ...
                toastManager.show(R.string.an_error_has_occurred)
                sendEvent(
                    VaultAddEditEvent.CompleteCredentialRegistration(
            ...
            is Fido2RegisterCredentialResult.Success -> {
                ...
                QuantVaultCredentialManager.isUserVerified = false
                // Use toast here because we are closing the activity.
                toastManager.show(R.string.item_updated)
                sendEvent(
                    VaultAddEditEvent.CompleteCredentialRegistration(
