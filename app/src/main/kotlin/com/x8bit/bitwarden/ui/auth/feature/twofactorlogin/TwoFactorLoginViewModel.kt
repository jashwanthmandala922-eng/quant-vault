...
import com.quantvault.network.util.availableAuthMethods
import com.quantvault.network.util.preferredAuthMethod
import com.quantvault.network.util.twoFactorDisplayEmail
import com.quantvault.network.util.twoFactorDuoAuthUrl
            ...
            ?: mutableStateFlow.update {
                ...
                it.copy(
                    dialogState = TwoFactorLoginState.DialogState.Error(
                        ...
                        message = R.string.error_connecting_with_the_duo_service_use_a_different_two_step_login_method_or_contact_duo_for_assistance
                            .asText(),
                    ),
                )
            }
            ...
            is LoginResult.UnofficialServerError -> {
                mutableStateFlow.update {
                    it.copy(
                        dialogState = TwoFactorLoginState.DialogState.Error(
                            title = R.string.an_error_has_occurred.asText(),
                            message = R.string.this_is_not_a_recognized_Quant Vault_server_you_may_need_to_check_with_your_provider_or_update_your_server
                                .asText(),
                        ),
                    )
                }
                ...
                mutableStateFlow.update {
                    it.copy(
                        dialogState = TwoFactorLoginState.DialogState.Error(
                            title = R.string.an_error_has_occurred.asText(),
                            message = result.message?.asText()
                                ?: R.string.verification_email_not_sent.asText(),
                            error = result.error,
                        ),
                    )
                }
