...
data class AccountSecurityState(
        ...
        get() = vaultTimeoutPolicy?.let { policy ->
            when (policy.type) {
                PolicyInformation.VaultTimeout.Type.NEVER -> {
                    R.string.your_organization_has_set_the_default_session_timeout_to_never
                        .asText()
                }
                ...
                PolicyInformation.VaultTimeout.Type.ON_SYSTEM_LOCK,
                    -> {
                    R.string.your_organization_has_set_the_default_session_timeout_to_on_app_restart
                        .asText()
                }

                PolicyInformation.VaultTimeout.Type.IMMEDIATELY -> {
                    R.string.this_setting_is_managed_by_your_organization.asText()
                }
                ...
                null,
                    ...
                    if (hours != null && minutes != null) {
                        R.string.vault_timeout_policy_in_effect_hours_minutes_format.asText(
                            R.plurals.hours_format.asPluralsText(hours, hours),
                            R.plurals.minutes_format.asPluralsText(minutes, minutes),
                        )
                    } else if (hours != null) {
                        R.string.vault_timeout_policy_in_effect_format.asText(
                            R.plurals.hours_format.asPluralsText(hours, hours),
                        )
    ...
    val sessionTimeoutActionSupportingText: Text?
        get() = if (vaultTimeoutPolicy?.action != null) {
            ...
            R.string.set_up_an_unlock_option_to_change_your_vault_timeout_action.asText()
        } else {
            null
        }
}
