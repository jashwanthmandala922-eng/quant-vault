...
class HeadersInterceptorTest {
    ...
    fun `intercept should modify original request to include custom headers`() {
        ...
        val headersInterceptors = HeadersInterceptor(
            userAgent = "quantvault_Mobile/$versionName ($buildType/$flavor) (Android $release; SDK $sdk; Model robolectric)",
            clientName = "mobile",
        ...
        @Suppress("MaxLineLength")
        assertEquals(
            "quantvault_Mobile/$versionName ($buildType/$flavor) (Android $release; SDK $sdk; Model robolectric)",
            request.header("User-Agent"),
        )
        assertEquals("mobile", request.header("quantvault-Client-Name"))
        assertEquals(versionName, request.header("quantvault-Client-Version"))
