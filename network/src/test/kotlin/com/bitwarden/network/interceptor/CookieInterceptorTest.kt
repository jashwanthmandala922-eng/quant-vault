...
class CookieInterceptorTest {
    ...
    fun `intercept should skip cookie handling for config path`() {
        val originalRequest = Request.Builder()
            .url("https://api.quantvault.com/api/config")
            .build()
    ...
    fun `intercept should skip cookie handling for sso-cookie-vendor path`() {
        val originalRequest = Request.Builder()
            .url("https://api.quantvault.com/api/sso-cookie-vendor")
            .build()
        val chain = FakeInterceptorChain(originalRequest)
    ...
    fun `intercept should throw CookieRedirectException when bootstrap is needed`() {
        val originalRequest = Request.Builder()
            .url("https://vault.quantvault.com/api/accounts/profile")
            .build()
        val chain = FakeInterceptorChain(originalRequest)

        every { mockCookieProvider.needsBootstrap("vault.quantvault.com") } returns true

        val exception = assertThrows<CookieRedirectException> {
            interceptor.intercept(chain)
        }

        assertEquals("vault.quantvault.com", exception.hostname)
        verify { mockCookieProvider.acquireCookies("vault.quantvault.com") }
    ...
    fun `intercept should proceed without cookies when no cookies available`() {
        ...
        every { mockCookieProvider.needsBootstrap("vault.quantvault.com") } returns false
        every { mockCookieProvider.getCookies("vault.quantvault.com") } returns emptyList()

        val response = interceptor.intercept(chain)

        assertEquals(originalRequest, response.request)
        assertNull(response.request.header("Cookie"))
        verify(exactly = 1) { mockCookieProvider.getCookies("vault.quantvault.com") }
    }
    ...
    fun `intercept should attach single cookie when cookies available`() {
        ...
        every { mockCookieProvider.needsBootstrap("vault.quantvault.com") } returns false
        every { mockCookieProvider.getCookies("vault.quantvault.com") } returns cookies
    ...
    fun `intercept should skip cookie handling for cloud config path`() {
        val originalRequest = Request.Builder()
            .url("https://api.quantvault.com/config")
            .build()
        val chain = FakeInterceptorChain(originalRequest)
