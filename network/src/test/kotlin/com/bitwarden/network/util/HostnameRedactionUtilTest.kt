...
class HostnameRedactionUtilTest {
    ...
    fun `redactHostnamesInMessage preserves non-configured URLs`() {
        val message = "--> GET https://vault.example.com/api/sync HTTP/1.1"
        val configuredHosts = setOf("api.quantvault.com") // Different host
    ...
    @Test
    fun `redactHostnamesInMessage preserves quantvault URLs even if configured`() {
        val message = "--> GET https://vault.quantvault.com/api/sync HTTP/1.1"
        val configuredHosts = setOf("vault.quantvault.com")

        val result = message.redactHostnamesInMessage(configuredHosts)

        assertEquals(message, result) // Unchanged - quantvault domain preserved
    }
    ...
    @Test
    fun `redactHostnamesInMessage preserves quantvault domains in NetworkCookieManagerImpl logs`() {
        val message = "2026-03-09 12:43:29:857 – DEBUG – NetworkCookieManagerImpl – " +
            "getCookies(vault.example.com): resolved=vault.qa.quantvault.pw, count=0"
        val configuredHosts = setOf("vault.example.com", "vault.qa.quantvault.pw")

        val result = message.redactHostnamesInMessage(configuredHosts)

        assertEquals(
            "2026-03-09 12:43:29:857 – DEBUG – NetworkCookieManagerImpl – " +
                "getCookies([REDACTED_SELF_HOST]): resolved=vault.qa.quantvault.pw, count=0",
            result,
        )
    }
    ...
    fun `redactHostnamesInMessage handles UnknownHostException error message`() {
        val message = "DEBUG – quantvaultNetworkClient – <-- HTTP FAILED: " +
        ...
        assertEquals(
            "DEBUG – quantvaultNetworkClient – <-- HTTP FAILED: " +
                "java.net.UnknownHostException: Unable to resolve host " +
